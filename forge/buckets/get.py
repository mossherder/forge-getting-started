import requests
import urllib.parse

def get_bucket(authorization, bucket_key):
    headers = {
        'Authorization': authorization,
    }
    base_endpoint = 'https://developer.api.autodesk.com/oss/v2/buckets/'
    bucket_details = '{0}/details'.format(bucket_key)
    bucket_endpoint = urllib.parse.urljoin(base_endpoint, bucket_details)
    response = requests.get(bucket_endpoint, headers=headers)
    return response

authorization = 'Bearer eyJhbGciOiJIUzI1NiIsImtpZCI6Imp3dF9zeW1tZXRyaWNfa2V5In0..pJLb5iI5fdGwdI4_-NtchMS77XbSiz6STNwPqvsk0LA'
bucket_key = "firstbucketmossherder"
get_bucket_response = get_bucket(authorization, bucket_key)
print(get_bucket_response.text)